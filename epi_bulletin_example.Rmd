...
```{r measles_plot, fig.width=7.5, fig.height=2.5, out.width="100%", out.height="100%"}
...
# Create a Single-Cell TableGrob with Text
...
flextable(sub_header_data) %>%
  ...
  border(border = officer::fp_border(color="white"), part="all") %>%
  bold(i = 2, j = 1:2) %>%
  fontsize(i = 2, j = 1:2, size = 12) %>%  # Adjust text size
...
maternal_death_plot <- ggplot(data = maternal_deaths_plot_data,
  ...
  geom_text(aes(label = scales::percent(pct, accuracy = 1)), 
            position = position_nudge(y = max(maternal_deaths_plot_data$total) * 0.04),  # Nudge labels outside
  ...
  theme(axis.title.y=element_blank(),
        panel.grid.major = element_line(color="grey90", linetype="dashed")) +
  scale_y_continuous(limits=c(0, max(maternal_deaths_plot_data$total, na.rm=T)+0.5))
...
maternal_death_map <-ggplot(data = map_data) +
  ...
  theme(
    ...
    legend.key.height = unit(0.15, "in"),  # Reduce legend height
    legend.key.width = unit(0.1, "in"),  # Make legend narrower
...
```{r maternal_deaths_plot_grid, fig.width=7.5, fig.height=3, out.width="100%", out.height="100%"}
cowplot::plot_grid(maternal_death_plot, maternal_death_map, align="h", nrow=1, rel_widths = c(4, 4.5))
```
